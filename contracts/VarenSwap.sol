...
contract VarenRouter is ReentrancyGuard {
	...
	bytes memory _data
        ...
        bytes memory data;
        (swapRecord.outToken, swapRecord.burnSendTo, data) = abi.decode(_data, (address, bytes, bytes));
    ...
    function swapTokensBack(SwapRecord storage record) internal returns (uint256 amountSwappedBack) {
      address[] memory path = new address[](3);
      path[0] = record.outToken;
      path[1] = weth;
      path[2] = record.inToken;
      uint256[] memory amounts = IUniswapV2Router02(sushiRouter).swapExactTokensForTokens(record.qty, 1, path, address(this), block.timestamp + 1);
      amountSwappedBack = amounts[amounts.length - 1];
    }

    function burnRenToken(SwapRecord storage swapRecord) internal {
        registry.getGatewayByToken(swapRecord.outToken).burn(swapRecord.burnSendTo, swapRecord.qty);
