...
contract VarenRouter is ReentrancyGuard {
    ...
    function receiveLoan(
        ...
        // Executing the swap
        if (
            IERC20(swapRecord.inToken).allowance(address(this), router) <
            _amount
        ) {
    ...
    function repayLoan(
        ...
        uint256 qty = record.qty;
        uint256 fee = FEE.mul(qty).div(PERCENTAGE_DIVIDER);
        uint256 qtyLessFee = qty.sub(fee);
        uint256 amountSwappedBack = swapTokens(
            record.outToken,
            record.inToken,
            fee
        );
        IERC20(record.inToken).safeTransfer(devWallet, amountSwappedBack);
        if (record.burnSendTo.length != 0) {
            burnRenToken(record);
        } else {
            IERC20(record.outToken).safeTransfer(record.to, qtyLessFee);
        }
    }
